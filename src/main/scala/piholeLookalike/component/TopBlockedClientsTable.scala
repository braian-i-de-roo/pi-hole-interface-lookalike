...
class TopBlockedClientsTable private(val node: Element)

object TopBlockedClientsTable {
  def apply(queries: EventStream[Query]): TopBlockedClientsTable = {
    ...
    new TopBlockedClientsTable(node)
  }
}
