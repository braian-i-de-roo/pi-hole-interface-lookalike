...
class TopBlockedDomainsTable private(val node: Element)

object TopBlockedDomainsTable {

  def apply(queries: EventStream[Query]): TopBlockedDomainsTable = {
    ...
    val columns = grouped.map{ m =>
      ...
      val aux =m.map{ tuple =>
        ...
        (tuple._1, count, ProgressBar(percent).node)
      }.toList.sortBy(-_._2)
      aux.map(x =>
        tr(
          th(x._1),
    ...
    new TopBlockedDomainsTable(node)
  }
}
